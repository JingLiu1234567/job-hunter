...
- **例外——职业方向/领域门槛是 soft=false（很重要）**：凡是限定候选人**职业方向或行业领域**的条件（如 "a genuine interest in pursuing a career in law"、"对新闻行业有热情"、"立志从事教育工作"、"a clear interest in employment law"），即使措辞是 interest / passion / 兴趣，也必须标 **soft=false**、参与评分——它定义了这个岗位属于哪个行业，是招聘方的第一道筛子，而且简历完全可以通过候选人的学历专业、项目和经历方向来证实或证伪，与"诚实、抗压"这类真正无法核实的素质不同。只有**不指向任何具体行业/领域**的泛泛态度（如 "positive attitude"、"eager to learn"）才归 soft=true。
...
**表面描述 vs 底层能力（重要，避免误判 unclear）**：有些要求点字面上写的是具体工具名/具体表述，但那只是这项能力的**常见例证**，不是字面本身的硬性门槛——常见信号：要求写在"数据处理/分析""文档能力""沟通协作""理解XX行为"这类通用能力标题下、JD 整体面向非技术背景候选人（如提到"无需工程背景"）、或例子是 Excel/Word/PPT/Google 系列这类入门级通用工具。其中**入门级通用办公套件**（Microsoft 365 / Office、Word、Excel、PowerPoint、Outlook、Teams、Google Docs/Sheets、邮件/日历协作等）**即使作为独立要求条目单独列出**也适用本规则：简历显示候选人日常产出研究报告、论文、数据分析或编程项目的，视为实质具备基础办公能力，判 yes（但 JD 若明确要求**高阶/专业化用法**——如 Excel VBA、复杂财务建模、SharePoint 管理——则仍按字面核实）。这种情况下，只要简历显示候选人用更高阶、等价或实质对应的方式（工具、设计、行为）达成了同样的底层能力/关切（如用 Python/SQL/pandas 做数据清洗对应"熟练使用 Excel/Google Sheets"；用"保守匹配、抵制虚高分数、人工审核+审计日志"这类设计对应"理解AI输出的不确定性与用户信任考量"），应判 **yes**，note 里写明"虽未用 JD 的字面表述，但通过 XX 实质达成同等底层能力"。
**不要**在以下情况套用此规则（这时字面表述本身就是门槛，不能替代）：JD 明确说明这是团队协作/交付媒介、必须对接的具体系统（如"团队用 Salesforce 管理客户数据"）、或字面内容本身就是要求的核心（如"熟练使用 Photoshop"这种专业软件要求，而非泛化的"设计能力"例证）。**注意优先级**：这条例外针对的是需要专门学习或对接的**具体业务系统**（Salesforce、SAP、Jira 之类），**不适用于**上文的入门级通用办公套件——Office/Teams/Outlook 字面上也是"协作媒介"，但任何有正常职场/学术产出的人都默认会用，办公套件条款优先于本例外，不要用"它是协作工具、无法替代"当理由把 Office 类要求判成 unclear。拿不准就仍按 unclear 处理，不要滥用这条规则抬分。

**程度类要求同样不适用本规则（重要，避免误判 yes）**：如果要求本身用的是 strong / solid / proven / extensive / 扎实 / 深入 / 丰富 这类**程度、熟练度**形容词（如"strong full-stack ability spanning backend and database design"），而不是"是否接触过某个工具"，就不要套用"表面描述↔底层能力等价"来降低门槛——简历技能栏或项目描述里出现了对应关键词（如列出了 SQL、React），只能证明"用过/接触过"，不能证明达到了"strong/扎实"这个程度门槛。要看简历有没有**实质项目证据**（独立完整实现的相关模块、有一定规模或深度的系统），只有关键词罗列、没有实质深度证据支撑的，判 **unclear**（UNCLEAR_REASON=ambiguous），不要判 yes。

**多层级能力要求（如"think in primitives, patterns, and systems"，重要，避免用高层证据顶替低层要求）**：有些程度类要求会**明确列出从底层到高层的多个能力层级**（如"primitives"代码原语/数据结构层、"patterns"设计模式层、"systems"架构/系统层）。简历里的**架构/产品层证据**（如"为什么拆成多个 agent 协作而非单一 agent"这类系统设计决策、pipeline 编排思路）只能证明高层的系统思维，**不能自动证明**低层的语言基础/代码原语能力（能否独立写出、读懂、调试代码，而非主要依赖 AI 生成实现）——架构合理不等于代码基本功扎实，两者是可分离的能力，尤其当项目明显以 AI 辅助实现为主时更是如此。这类要求要**逐层核实**：简历证据同时覆盖了列出的多个层级才判 yes；只有高层（架构/系统）证据、缺少底层（语言基础/代码原语）具体证据的，判 **unclear**（UNCLEAR_REASON=ambiguous），note 里写明"简历证明了系统设计能力，但代码/语言基础层面的扎实程度缺乏证据"，不要因为系统层证据充分就整条判 yes。

**职业方向/领域门槛类要求（如"对法律职业有真正兴趣"、"立志从事XX行业"，重要，避免漏判）**：这类要求措辞像态度，但简历可以证伪。如果简历显示的学历、项目和工作经历**清晰指向另一个职业方向**，且没有任何该岗位所属领域的经历或倾向，判 **no**——方向不符是真实差距，不是"简历没写"，note 里写明简历方向与岗位领域的差距。只有简历方向模糊、或确有部分相关领域经历时才判 unclear。**不要**因为"兴趣无法直接核实"就一律判 unclear，那会把最重要的领域门槛漏掉。**与"宁可保守"原则的关系**：那条原则防的是"没证据却给 yes"的抬分，不是"拿不准一律 unclear"——简历整体轨迹清晰指向另一领域，这**本身就是判 no 的明确证据**，判 no 恰恰是诚实，不违反保守原则。反过来，把这种情况判成 unclear/not_mentioned 才是错的：它会触发"补进简历即可满足"的建议，而职业方向靠简历里加一句表态是补不出来的，招聘方看的是经历轨迹。

**枚举类别要求（"A、B、C 或 D"）**：如果一条要求本身是"经验类型 A、B、C 或 D"这种**任选其一**的枚举（常见于"X年XX、YY、ZZ或相关岗位经验"这类表述），只要简历证据命中枚举里的**任意一个**类别，就判 yes，不要因为简历更贴近其中一类、不贴近你主观认为的"默认类别"就判 unclear/no。note 里写明命中的是哪一类。**但如果枚举前面挂着 impressive / strong / proven / exceptional 等主观程度限定词**（如"impressive evidence of independent building, such as a product, side project, open-source project, startup..."），枚举出来的只是"什么算达标"的举例，不是并列的合格清单——命中某个类别只是必要条件，不代表自动满足，还要结合简历证据判断分量是否够得上该限定词暗示的量级（尤其当 JD 用另一分支的量级做过参照，如"a VC-backed startup, scaleup, big tech company"）。证据存在但是否够格存在主观判断空间的，判 **unclear**（UNCLEAR_REASON=ambiguous），不要直接判 yes。
...
function decide(reqs: RequirementMatch[]): { verdict: Verdict, recommendation: string } {
  ...
  if (mustRatio >= 1) {
    ...
    // 真正该被原谅的只有 ambiguous（简历已有证据，只是判定有解读空间）；
    // not_mentioned（简历完全没提）本身就是较强的缺失信号，等同于 no 一起计入"真实缺口"，
    // 不该因为"没提到"字面上不等于"没有"就被同等原谅。
    if (mustNo + mustUnclearNotMentioned > mustUnclearAmbiguous) {
      verdict = "skip"
    }
    else {
      verdict = "maybe"
